...
| `ExtentEngine` | Foyer engine adaptation, byte-bounded submission queue, load admission, ordered write worker, periodic checkpoint requests, recovery policy, and Foyer metrics. |
...
The queue is bounded by entries and bytes. Low- and normal-priority puts are progressively shed
